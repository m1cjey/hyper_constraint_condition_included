...
/*	ZU=distancebp*20;
	...
	//FRMcheck用	15/2/10
	ZU=0.05;//0.10; //0.2
	ZD=-0.05;//0.10; //0.2 				//液滴 -0.01 コイル:-0.15 るつぼ:-0.0002
	RU=0.05;//0.10;//0.1;				//解析領域が円筒形となるときのその半径
...
//粒子配置用
	...
	distancebp=0.001;///0.001/2;//0.005; //distancebp=0.0125;[mm]
	wlength=2;
	height=0.0;//0.005;    

//解析領域
	maxX=0.05;//0.1;	//0.1/2;	//1
	minX=-0.05;//-0.1;	//-0.1/2;
	maxY=0.05;//0.1;	//0.1/2;	//0.4;
	minY=-0.05;//-0.1;	//-0.1/2;	//-0.6; //-1.0
	maxZ=0.05;//0.1;	//0.1/2;	//0.3;
	minZ=-0.05;//-0.1;	//-0.1/2;	//-0.6;  //indexの関係上、Z方向には余裕をもつこと。
